...
export default function ContributeGoalPage() {
  ...
  if (!goal) return <div className="container-app"><p>Loading...</p></div>;
  ...
  return (
    <div className="container-app">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-primary)' }}>Add Funds</h1>
          <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>Contributing to: <strong>{goal.emoji} {goal.name}</strong></p>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="card" style={{ padding: '24px' }}>
        ...
        {remainingAmount > 0 && (
          <div style={{ marginBottom: '20px', padding: '12px', background: 'var(--bg-input)', borderRadius: 'var(--radius-md)', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
            Remaining to target: <strong style={{ color: 'var(--accent-5)' }}>{remainingAmount.toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })}</strong>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div>
            ...
            <input style={inputStyle} value={formData.note} onChange={e => setFormData({ ...formData, note: e.target.value })} placeholder="e.g. November savings" />
          </div>
